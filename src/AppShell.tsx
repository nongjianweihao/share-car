...
const renderCardContent = (card: Card | undefined) => {
    if (!card) {
        return <p>请选择一个卡片</p>;
    }

    const componentBlock = card.blocks.find(block => block.type === 'component');
    ...
    return (
        <article className="card-article">
            ...
            {card.blocks.map(renderGenericBlock)}
        </article>
    );
};
...
export const AppShell: React.FC = () => {
    ...
    return (
        <div className="app-container">
            ...
            <main className="main-content">{renderCardContent(activeCard)}</main>
        </div>
    );
};
